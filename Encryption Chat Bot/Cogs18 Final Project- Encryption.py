    ...
    def variable_encoder(the,crypt):
       ...
       #this is a counter for the while loop to ensure it doesn't loop infinitely 
        ...
        #some may say I may have went a little overboard, BUT NAY! I MUST DO EVERYTHING I CAN TO PROTECT MY PRECIOUS ENCRYPTION!!!
                ...
                encrypted = random.choice(banter) + finishd + encoded
...
def secret_messenger():
    """starts a chatbot that will loop continuously until you input stop/exit.
    ...
    it will continue to loop until there is either an error or you input stop/exit"""
    
    #these are phrases the bot will say at random whenever it is encoding your message.
    talk = ['alright getting your message ready...',
          "ok,that's an interesting message...",
          "don't worry, I won't tell anyone..."]
    
    #this is here so the variable repeat is set. It will be used to say a different message if you have already encoded.
    repeat = None
    
    #these are the safe words, upon saying any of these, the chatbot will shut down.
    pls_stop = ['stop', 'no', 'exit', 'get me outta here', "i've had enough", 'done', 'im done']
    
    #this variable sets up a question regarding what you want to do
    purpose = 'Would you like to encode a message, or decode one you have? you can simply enter "e" or "d" as well: '
    
    #This ensures the while loop which keeps the bot alive, is running
    chat = True
    
    #this also sets up a variable for later use, that must be assigned before, Ill get into what it does then.
    decrypt_pass = False
    
    #this introduces the bot and as such, it is placed before the loop begins
    welcome = input("Hi! I'm here to encode and/or decode your message! To get started press enter. If you want to leave at anytime, simply type in no, stop, or exit. ")
    if welcome in pls_stop:
        chat = False
        print('pls gimme a chance')
    
    #loops the chatbot
    while chat:
        
        #repeat is true when you've encoded, allowing a new message acknowledging that fact
        if repeat == True:
            purpose = 'alright, would you like to encode another message? or decode? or are you done?: '
            
            #changes repeat back to normal so the message printed is the normal version the next time its looped
            repeat = None
        
        print()
        #allows users to skip to encoding function directly from decoding
        if decrypt_pass is True:
            your_purpose = 'encode'
            decrypt_pass = False
            ...
            #stores your input
            your_purpose = input(purpose)
        
        print()
        
        #sets the question the bot will ask when looped
        purpose = 'Would you like to encode a message, or decode one you have?: '
        
        #allows users to simply input e to encode
        if your_purpose=='e':
            your_purpose=your_purpose+'encode'
        
        #allows users to simply input d to decode        
        elif your_purpose=='d':
            your_purpose=your_purpose+'decode'
        
        #will run the encoder function if input was encode
        if 'encode' in your_purpose:
            
            #will attempt to encode the message
            try:
                new_msg_enc=encoding_time()
            
            #if encoding fails, continues the loop
            except:
                print()
                print('uh oh, looks like something went wrong. If you have entered really big numbers then I will get confused. Sorry! please enter different keys that are smaller next time')
                continue
            
            #stores the message and the keys used
            actual_message = str(new_msg_enc[0])
            key1=int(new_msg_enc[1])
            key2=int(new_msg_enc[2])
            key3=int(new_msg_enc[3])
            print()
            ...
            print('and here are your 3 keys in case you forget: ')
            
            #prints all 3 keys used
            print('key: ',key1)
            print('key increment: ',key2)
            print('key increment changer: ',key3)
            
            #asks if user wants to immediately decode the message
            im_decode=input('would you like to decode this immediately? ')
            print()
            
            #begins decoding message if input was yes.
            if 'yes' in im_decode:
                
                #assigns parameters returned from the Impossible class to the MakeTheImpossiblePossible class
                immediate_decode = MakeTheImpossiblePossible(actual_message, key1, key2, key3)
                
                #returns the decoded message
                dec_msg = immediate_decode.decoder()
                
                print('Here is the original message: ', dec_msg)
                print()
            
            #changes the question the bot asks at the beginning to acknowledge that you've just encoded
            repeat = True
        
        #runs the decoder function if user inputed decode
        elif 'decode' in your_purpose:
            dec_msg = decoding_time()
            
            #allows the next while loop iteration to skip inputs and go straight to the encoder
            if dec_msg is None:
                decrypt_pass = True
                continue
            
            #prints the original message after decoding
            print('Here is the original message: ')
            print(dec_msg)
        
        #exits the loop and closes the chatbot if inputed a message in pls_stop
        elif your_purpose in pls_stop:
            
            print('ok! remember to stay safe and enjoy the rest of your day! bye!')
            chat = False
        
        #informs the user to input either encode or decode and continues looping
        else:
            
            print('I only want to know if you want to encode or decode, anything beyond that, is also beyond my pay grade.')
...
def encoding_time():
    ...
    while start_key not in num_limit:
        ...
        if start_key >= 1114111:
            while start_key >= 1114111:
                start_key = int(input('WOAH! That is a huge number, can you please type in smaller number?: '))
        
        #notifies user value is too small
        if start_key <= 0:
            while start_key <= 0:
                start_key = int(input('Please no negative numbers, this includes 0, please enter a bigger number: '))
    ...
    while key_increment not in num_limit:
        ...
        #notifies user value is too big
        if key_increment >= 1114111:
            while key_increment >= 1114111:
                key_increment = int(input('WOAH! That is a huge number, can you please type in smaller number?: '))
        
        #notifies user value is too small
        if key_increment <= 0:
            while key_increment <= 0:
                key_increment = int(input('Please no negative numbers, this includes 0, please enter a bigger number: '))
    ...
    while key_increments_increment not in num_limit:
        ...
        if key_increments_increment >= 1114111:
            while key_increments_increment >= 1114111:
                key_increments_increment = int(input('WOAH! That is a huge number, can you please type in smaller number?: '))
        
        #notifies user value is too small
        if key_increments_increment <= 0:
            while key_increments_increment <= 0:
                key_increments_increment = int(input('Please no negative numbers, this includes 0, please enter a bigger number: '))
...
def decoding_time():
    ...
    #asks to input yes or no to continue to decode
    do_you = input('You want a message decoded? Remember I can only decode messages that were decoded by me! Have you copied a message that was encoded by me and do you also remember all 3 keys you entered? enter Yes or No: ')
    
    #allows interpretation regardless of capitilization
    interpret = do_you.lower()
    
    #continues decoding if input was yes
    if interpret == 'yes':
        
        print()
        
        #takes the input of the encoded message
        msg_to_dec = input('Great! Please paste the encoded message in its entirity here: ')
        
        #takes the starting key value
        key1 = int(input('OK, now enter the secret key, which was the first one you entered: '))
        
        #takes the key_increment value
        key2 = int(input('Cool, now enter the key increment, which was the second key you entered: '))
        
        #takes the key_increments_increment value
        key3 = int(input('allright! finally, enter the key increment changer, which is the last, third, key you entered: '))
    ...
    #redirects user to the encoder to encode a message if they answered no
    elif interpret == 'no':
        
        print("OK, I'll redirect you to the encoder, remember the keys you enter, and to copy the entire code.")
        
        #allows the next while loop iteration to skip inputs and go straight to the encoder
        return None
    
    #loops until user enters a valid response
    else:
